...
# sample function
def do_sample(train_config, accelerator, ckpt_path=None, cfg_scale=None, ig_scale=None, model=None, vae=None, demo_sample_mode=False):
    """
    Run sampling.
    """
    ...
    if ig_scale is None:
        ig_scale = train_config['sample']['ig_scale']
    ...
    if accelerator.process_index == 0:
        ...
        print_with_prefix('ig_scale=', ig_scale)
    ...
    if demo_sample_mode:
        ...
        for i in pbar:
            ...
            if using_cfg:
                ...
                y = torch.cat([y, y_null], 0)
                model_kwargs = dict(y=y, cfg_scale=cfg_scale, ig_scale=ig_scale, cfg_interval=True, cfg_interval_start=cfg_interval_start)
                model_fn = model.forward_with_cfg
            else:
                model_kwargs = dict(y=y, ig_scale=ig_scale)
                model_fn = model.forward_with_ig
