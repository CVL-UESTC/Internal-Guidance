...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        self.final_layer_xr = FinalLayer(hidden_size, patch_size, self.out_channels, use_rmsnorm=use_rmsnorm)
    ...
    def forward(self, x, t, y):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if (i + 1) == self.encoder_depth:
                # zsr = [projector(x.reshape(-1, D)).reshape(N, T, -1) for projector in self.projectors]
                zsr = self.final_layer_xr(x, c)
                zsr = self.unpatchify(zsr)
        ...
        return x, zsr
    def forward_with_ig(self, x, t, y, ig_scale, ig_interval=1):
        model_out, zsr = self.forward(x, t, y)
        if ig_interval > 0:
            timestep = t[0]
            if timestep <= ig_interval:
                model_out = model_out + (ig_scale-1)*(model_out - zsr)
        return model_out
    def forward_with_cfg(self, x, t, y, cfg_scale, ig_scale, cfg_interval=None, cfg_interval_start=None, ig_interval=1):
        """
        ...
        model_out, zsr = self.forward(combined, t, y)
        if ig_interval > 0:
            timestep = t[0]
            if timestep <= ig_interval:
                model_out = model_out + (ig_scale-1)*(model_out - zsr)
...
LightningDiT_models = {
    ...
    'LightningDiT-L/2': LightningDiT_L_2,'LightningDiT-L/1': LightningDiT_L_1,
