...
def euler_maruyama_ig_sampler(
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-2], t_steps[1:-1])):
            ...
            if sg_scale > 1.0 and t_cur <= sg_guidance_high and t_cur >= sg_guidance_low:
                v_cur = v_cur_s.to(torch.float64) + sg_scale*(v_cur.to(torch.float64) - v_cur_s.to(torch.float64))
            else:
