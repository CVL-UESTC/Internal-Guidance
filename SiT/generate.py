...
import math
import argparse
from samplers import euler_sampler, euler_maruyama_sampler, euler_maruyama_ig_sampler
...
def main(args):
    ...
    if ckpt_path is None:
        ...
        try:
            ...
            state_dict = torch.load(ckpt_path, map_location=f'cuda:{device}')['ema']#ema model
        except:
    ...
    folder_name = f"{exp_name}-{ckpt_string_name}-{args.resolution}-vae-{args.vae}-" \
                  f"cfg-{args.cfg_scale}-cfgh-{args.guidance_high}-ig-{args.ig_scale}-iglow{args.ig_guidance_low}-igh{args.ig_guidance_high}-seed-{args.global_seed}-{args.mode}" #-noema
    ...
    for _ in pbar:
        ...
        sampling_kwargs = dict(
            ...
            sg_guidance_low=args.ig_guidance_low,
            sg_guidance_high=args.ig_guidance_high,
            path_type=args.path_type,
        )
        with torch.no_grad():
            if args.mode == "sde":
                samples = euler_maruyama_ig_sampler(**sampling_kwargs).to(torch.float32)
            elif args.mode == "ode":
...
if __name__ == "__main__":
    ...
    # model
    parser.add_argument("--model", type=str, choices=list(SiT_models.keys()), default="SiT-B/2") #SiT_models.keys() DiC LightningDiT_models.keys()
    parser.add_argument("--num-classes", type=int, default=1000) #50
    ...
    parser.add_argument("--num-fid-samples", type=int, default=50_000) #50_000

    # sampling related hyperparameters
    parser.add_argument("--mode", type=str, default="sde")
    parser.add_argument("--cfg-scale",  type=float, default=1.8)
    parser.add_argument("--ig-scale",  type=float, default=1.0)
    ...
    parser.add_argument("--ig-guidance-low", type=float, default=0.)
    parser.add_argument("--ig-guidance-high", type=float, default=1.)
